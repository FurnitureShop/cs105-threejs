...
export default function HomePage(scene: THREE.Scene, camera: THREE.Camera) {
	...
	const eventRotateBlockOnHold = (event: MouseEvent) => {
		...
		cube.rotation.y += holdSwipeDistance / (window.innerWidth / 10);
	};

	const maximumRotationDistance = Math.PI / 2;
	...
	document.addEventListener("mouseup", (event) => {
		...
		// };
		// animate();
		document.removeEventListener("mousemove", eventRotateBlockOnHold, false);
	});
}
